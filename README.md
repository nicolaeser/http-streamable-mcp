...
- `src/resources/`: Static data and MCP App UIs / Statische Daten und MCP App Benutzeroberflächen.
- `src/resource-templates/`: Dynamic URI resources (e.g. `notes/{id}`) / Dynamische URI-Ressourcen (z. B. `notes/{id}`).
- `src/prompts/`: Reusable instruction sets / Wiederverwendbare Anweisungen.
- `src/mcp/`: Protocol, registry, loaders, sessions & task persistence / Protokoll, Registry, Loader, Sessions & Task-Persistenz.
- `src/http/`: Express server & Auth logic / Express Server & Authentifizierung.
- `src/runtime/`: Logger & Redis client / Logger & Redis-Client.
- `src/config/`: Environment configuration / Konfiguration der Umgebung.
